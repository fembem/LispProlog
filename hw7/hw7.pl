...
me_plan(Start, Goal, _, [], Start, Depth) :- 	%when we reach the goal the final end state will be our present 'start'
	subset(Goal, Start), !, %only look for the first solution
	printDepth(Depth), write('Goal '), print_stack(Goal), 
...
me_plan(Start, Goal, Been_list, [Name|Plan], End_state, Depth) :-
    New_depth is Depth + 1,
    printDepth(Depth), write('plan from '), print_stack(Start), 
    write(' to '), print_stack(Goal), nl,
    set_diff(Goal, Start, Diff_set),
    printDepth(Depth), write('Diff_set = '), print_stack(Diff_set), nl,
    ...
    printDepth(Depth), write(add(Diff)), write(' in actions'), nl,
    subset(Preconditions, Start),
    %write('Preconditions < Start'), nl,
    printDepth(Depth), write('Satisfying move = '), write(Name), nl,
    change_state(Start, Actions, Child_state),
    printDepth(Depth), write('Child_state = '), print_stack(Child_state), nl,
    not(member_state(Child_state, Been_list)),
    printDepth(Depth), write('Child_state not in been list'), nl, 
    stack(Child_state, Been_list, New_been_list),
    me_plan(Child_state, Goal, New_been_list, Plan, End_state, New_depth),!.	%just find first plan
...
me_plan(Start, Goal, Been_list, Plan, End_state, Depth) :-
    New_depth is Depth + 1,
    ...
    printDepth(Depth), write('2--preconditions '), print_stack(Preconditions), 
    write(' not a subset of start '), print_stack(Start), nl,
    me_plan(Start, Preconditions, Been_list, Sub_plan, Sub_end_state, New_depth),!,	%just find first plan
    %write('2--Diff_set = '), print_stack(Diff_set), nl,
    printDepth(Depth), write('2--Sub_plan = '), print_stack(Sub_plan), nl,
    printDepth(Depth), write('2--Sub_end_state = '), print_stack(Sub_end_state), nl,
    change_state(Sub_end_state, Actions, Child_state),
    printDepth(Depth), write('2--Child_state = '), print_stack(Child_state), nl,
    stack(Child_state, Been_list, New_been_list),
    append(Sub_plan, [Name], Pre_moves),
    printDepth(Depth), write('2--Pre_moves = '), print_stack(Pre_moves), nl,
    me_plan(Child_state, Goal, New_been_list, Post_moves, End_state, New_depth),!,	%just find first plan
    printDepth(Depth), write('2--Post_moves = '), print_stack(Post_moves), nl,
    append(Pre_moves, Post_moves, Plan),
    printDepth(Depth), write('2--append '), print_stack(Pre_moves), 
