...
printAssignments2(Assignments) :- write('{'), printAssignmentsRec2(Assignments).
printAssignmentsRec2([Assignment | []]) :- write(Assignment), write('}').
printAssignmentsRec2([Assignment | OtherAssignments]) :- write(Assignment), write(','), 
														printAssignmentsRec2(OtherAssignments).


%assign the first task to the first person in the people list
assign_first_task2([Task_name, Task_class, Task_time], 						%the first task
...
assign_first_task2(Task, [Unsuitable_Person|Other_people], Assignments, New_people, New_assignments) :-
    assign_first_task2(Task, Other_people, Assignments, New_people_2, New_assignments_2),
...
constrain2([], People, Assignments) :- write('success!! '), nl,
						write('Final people: '), write(People), nl,
						write('Final assignments: '), printAssignments2(Assignments), nl.
...
constrain2([ Task | Other_tasks], People, Assignments ) :-
    			assign_first_task2(Task, People, Assignments, New_people, New_assignments),
    			constrain2(Other_tasks, New_people, New_assignments ).
